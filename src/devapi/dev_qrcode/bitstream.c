...
static int BitStream_allocate(BitStream *bstream, int length)
{
	...
	bstream->length = length;	//length初始化	
	bstream->data = data;		//data初始化
...
static BitStream *BitStream_newFromNum(int bits, unsigned int num)
{
	...
	bstream = BitStream_new();	//分配BitStream结构体	
	
	if(bstream == NULL) 
		return NULL;

	if(BitStream_allocate(bstream, bits))	//初始化BitStream结构体
	{
...
static BitStream *BitStream_newFromBytes(int size, unsigned char *data)
{
	...
	if(BitStream_allocate(bstream, size * 8))	//初始化BitStream结构体	
	{
		BitStream_free(bstream);
		return NULL;
	}
...
int BitStream_append(BitStream *bstream, BitStream *arg)
{
	...
	bstream->length += arg->length;		//长度相加
	bstream->data = data;	//数据追加
...
unsigned char *BitStream_toByte(BitStream *bstream)
{
	...
	size = BitStream_size(bstream);		//返回length
