...
const QR_VERSIONINFO QR_VersonInfo[] = {{0}, 	// Ver.0
										 { 1, 	// Ver.1
										    26,   19,   16,   13,    9,		//码字总数，4种纠错等级下的数据码字
										   0,   0,   0,   0,   0,   0,   0,	//校正图形个数及坐标
										   1,  26,  19,		//纠错块1
										   1,  26,  16,
										   1,  26,  13,
										   1,  26,   9,
										   ...
										   0,   0,   0},
										 { 2, 	// Ver.2
										    44,   34,   28,   22,   16,		//码字总数，4种纠错等级下的数据码字
										   1,  18,   0,   0,   0,   0,   0,	//校正图形个数及坐标
										   1,  44,  34,		//纠错块1
										   1,  44,  28,
										   1,  44,  22,
										   1,  44,  16,
										   ...
										   0,   0,   0},
										 { 3, 	// Ver.3
										    70,   55,   44,   34,   26,		//码字总数，4种纠错等级下的数据码字	
										   1,  22,   0,   0,   0,   0,   0,	//校正图形个数及坐标
										   1,  70,  55,		//纠错块1
										   1,  70,  44,
										   2,  35,  17,
										   2,  35,  13,
...
const unsigned char	 nIndicatorLenNumeral[]  = 	 {10, 12, 14};		//数字模式下各版本组的位数
const unsigned char	 nIndicatorLenAlphabet[] = 	 { 9, 11, 13};		//字母数字模式下各版本组的位数
const unsigned char	 nIndicatorLen8Bit[]	 =   { 8, 16, 16};		//字节模式下各版本组的位数
const unsigned char	 nIndicatorLenKanji[]	 =   { 8, 10, 12};		//日文模式下各版本组的位数

int m_nSymbleSize;	//正方形的边长	

unsigned char	 m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE];	//最大正方形(125)的数据填充
//char **m_byModuleData;

int m_ncDataCodeWordBit;	//数据码的总位数

unsigned char	 m_byDataCodeWord[MAX_DATACODEWORD];	//存放所有的数据码

int m_ncDataBlock;	//连续编码模式的个数

unsigned char	 m_byBlockMode[MAX_DATACODEWORD];	//存放连续的编码模式

unsigned short	 m_nBlockLength[MAX_DATACODEWORD];	//连续单个编码模式的个数

int m_ncAllCodeWord;	//数据码和纠错码的总个数 

unsigned char	 m_byAllCodeWord[MAX_ALLCODEWORD];	//存放所有的数据码和纠错码 
...
{  
	...
	a = (char **)malloc( MAX_MODULESIZE * sizeof(char *) ); 
	
    a[0] = (char *)malloc( MAX_MODULESIZE * MAX_MODULESIZE * sizeof(char) );  
	
    for(i=1;i<MAX_MODULESIZE;i++)  
    {  
        a[i] = a[i-1] + MAX_MODULESIZE;          
    }  
	
    if(a!=NULL)
    {
		dev_debug_printf("mbyModuleData is ok!\r\n");
		dev_debug_printf("a=%x\r\n",a);
	...
	{
		dev_debug_printf("mbyModuleData is failure!\r\n");
	}
...
int GetBitLength(unsigned char nMode, int ncData, int nVerGroup)
{
	...
	switch (nMode)
	{
	case QR_MODE_NUMERAL:	//数字模式
		
		ncBits = 4 + nIndicatorLenNumeral[nVerGroup] + (10 * (ncData / 3));		//nIndicatorLenNumeral[nVerGroup]:表示数字个数的位数
	...
	default:		
		ncBits = 4 + nIndicatorLenKanji[nVerGroup] + (13 * ncData);	
		break;
	}
...
int SetBitStream(int nIndex, unsigned short wData, int ncData)
{
	...
	if (nIndex == -1 || nIndex + ncData > MAX_DATACODEWORD * 8)		//3200位
		return -1;
	...
	for (i = 0; i < ncData; ++i)
	{
		...
		{
			m_byDataCodeWord[(nIndex + i) / 8] |= 1 << (7 - ((nIndex + i) % 8));	//置1
		}
	}
...
void FormatModule(void)
{
	...
	SetFunctionModule();	//设置功能性图案

	SetCodeWordPattern();	//数据码和纠错码交替放在一起	

	if (m_nMaskingNo == -1)
	{
		m_nMaskingNo = 0;

		SetMaskingPattern(m_nMaskingNo);	//设置遮蔽信息
		
		SetFormatInfoPattern(m_nMaskingNo);	//设置格式信息图案 
...
void SetFunctionModule(void)
{
	...
	SetVersionPattern();	//设置版本信息图案

	/*设置校正图形图案*/
	for (i = 0; i < QR_VersonInfo[m_nVersion].ncAlignPoint; ++i)
	{
		SetAlignmentPattern(QR_VersonInfo[m_nVersion].nAlignPoint[i], 6);	//和3个大回字在一条直线上
		
		SetAlignmentPattern(6, QR_VersonInfo[m_nVersion].nAlignPoint[i]);	//和3个大回字在一条直线上
...
{
	static unsigned char byPattern[] = {0x7f,0x41,0x5d,0x5d,0x5d,0x41,0x7f}; 
...
{
	static unsigned char byPattern[] = {0x1f,0x11,0x15,0x11,0x1f}; 
...
void SetVersionPattern(void)
{
	...
	nVerData = m_nVersion << 12;	//版本号在高6位
...
int EncodeSourceData(char *lpsSource, int ncLength, int nVerGroup)
{
	...
	for (m_ncDataBlock = i = 0; i < ncLength; ++i)
	{
		...
		if (i < ncLength - 1 && IsKanjiData(lpsSource[i], lpsSource[i + 1]))
			byMode = QR_MODE_KANJI;				//3,日文汉字				
			
		else if (IsNumeralData(lpsSource[i]))	
			byMode = QR_MODE_NUMERAL;			//0,数字
			
		else if (IsAlphabetData(lpsSource[i]))
			byMode = QR_MODE_ALPHABET;			//1,字母数字
			
		else
			byMode = QR_MODE_8BIT;				//2,字节    QR_MODE_8BIT

		if (i == 0)
			m_byBlockMode[0] = byMode;			

		if (m_byBlockMode[m_ncDataBlock] != byMode)		//根据数据块，填充数据编码模式
			m_byBlockMode[++m_ncDataBlock] = byMode;

		++m_nBlockLength[m_ncDataBlock];	//根据数据块，填充数据数量

		if (byMode == QR_MODE_KANJI)		//如果是日文汉字模式，那么前后2个字节为一个汉字
		{
	...
	dev_debug_printf("m_ncDataBlock = %d\r\n", m_ncDataBlock);

	for (i = 0; i < m_ncDataBlock; ++i)
	{
		dev_debug_printf("m_byBlockMode[%d] = %d\r\n", i,m_byBlockMode[i]); 
		dev_debug_printf("m_nBlockLength[%d] = %d\r\n", i,m_nBlockLength[i]);		
	}
	...
	while (nBlock < m_ncDataBlock - 1)
	{
		...
		//如果前后2个数据块为字母和数字，可以合并成字母数字模式(字母模式包含了数字)  
			...
			(m_byBlockMode[nBlock] == QR_MODE_ALPHABET && m_byBlockMode[nBlock + 1] == QR_MODE_NUMERAL) )
		{
			/*******************************************
			 *计算数据位个数
			 *m_byBlockMode[nBlock]:数据编码模式，数字、字母数字、日文汉字
			 *m_nBlockLength[nBlock]:数据个数
			 *nVerGroup:版本组			
			 ******************************************/
			...
			ncDstBits = GetBitLength(QR_MODE_ALPHABET, m_nBlockLength[nBlock] + m_nBlockLength[nBlock + 1], nVerGroup);		//合并

			if (ncSrcBits > ncDstBits)	//如果合并前大于合并后长度
			{
				if (nBlock >= 1 && m_byBlockMode[nBlock - 1] == QR_MODE_8BIT)	//判断之前的数据块是不是8bit模式，如果是的话合并数据(必须要有3个数据块以上判断)
				{
				...
				if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_8BIT)	//如果后面的数据块为8bit模式，合并数据(必须要有3个数据块以上判断)
				{
				...
				if (nJoinPosition != 0)		//如果合并后的数据不为0
				{
				...
				else	//合并后数据异常，或者不满足合并条件
				{
					if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_ALPHABET)	//如果后面一块数据也是字母数字，合并数据
					{
	...
	{		
		ncSrcBits = GetBitLength(m_byBlockMode[nBlock], m_nBlockLength[nBlock], nVerGroup)
					+ GetBitLength(m_byBlockMode[nBlock + 1], m_nBlockLength[nBlock + 1], nVerGroup);	//合并2个部分数据
		...
		{//如果合并后数据增加，前一块数据是8bit数据
			...
			if (nBlock < m_ncDataBlock - 2 && m_byBlockMode[nBlock + 2] == QR_MODE_8BIT)	//数据块+2也是8bit
			{
				m_nBlockLength[nBlock + 1] += m_nBlockLength[nBlock + 2];
	...
	for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i)
	{
		...
		if (m_byBlockMode[i] == QR_MODE_NUMERAL)	//如果数据块是数字模式
		{
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 1, 4); 	//0001 数字模式

			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned short)m_nBlockLength[i], nIndicatorLenNumeral[nVerGroup]);	//数字个数
			...
			for (j = 0; j < m_nBlockLength[i]; j += 3)
			{
				if (j < m_nBlockLength[i] - 2)
				{
					wBinCode = (unsigned short)(((lpsSource[ncComplete + j]	  - '0') * 100) +
									  ((lpsSource[ncComplete + j + 1] - '0') * 10) +
									   (lpsSource[ncComplete + j + 2] - '0'));	//组成3位数
				...
				else if (j == m_nBlockLength[i] - 2)
				{
					wBinCode = (unsigned short)(((lpsSource[ncComplete + j] - '0') * 10) +
									   (lpsSource[ncComplete + j + 1] - '0'));	//组成2位数

					m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, wBinCode, 7);
				}
				else if (j == m_nBlockLength[i] - 1)
				{
					wBinCode = (unsigned short)(lpsSource[ncComplete + j] - '0');	//组成1位数

					m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, wBinCode, 4);
				}
			}

			ncComplete += m_nBlockLength[i];	//方便排序
			
			dev_debug_printf("m_ncDataCodeWordBit数字 = %d\r\n", m_ncDataCodeWordBit);
		}

		else if (m_byBlockMode[i] == QR_MODE_ALPHABET)
		{
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 2, 4);	//0010 字母数字模式

			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned short)m_nBlockLength[i], nIndicatorLenAlphabet[nVerGroup]);	//字母数字个数
			...
			for (j = 0; j < m_nBlockLength[i]; j += 2)
			{
				if (j < m_nBlockLength[i] - 1)
				{
					wBinCode = (unsigned short)((AlphabetToBinaly(lpsSource[ncComplete + j]) * 45) +
									   AlphabetToBinaly(lpsSource[ncComplete + j + 1]));	//组成2位数

					m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, wBinCode, 11);
				}
				else
				{
					wBinCode = (unsigned short)AlphabetToBinaly(lpsSource[ncComplete + j]);	//组成1位数
			...
			dev_debug_printf("m_ncDataCodeWordBit字母 = %d\r\n", m_ncDataCodeWordBit);
		}
		...
		else if (m_byBlockMode[i] == QR_MODE_8BIT)
		{
			...
			//dev_debug_printf("m_nBlockLength[%d] = %d\r\n", i,m_nBlockLength[i]);
			dev_debug_printf("m_ncDataCodeWordBit8位 = %d\r\n", m_ncDataCodeWordBit);
		}
		else	// if(m_byBlockMode[i] == QR_MODE_KANJI)	//日文中文模式
		{
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 8, 4);	//1000 日文中文模式

			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned short)(m_nBlockLength[i] / 2), nIndicatorLenKanji[nVerGroup]);	//日文中文个数
			...
			dev_debug_printf("m_ncDataCodeWordBit日文汉字 = %d\r\n", m_ncDataCodeWordBit);
		}
/*		else	//中文模式
		{
			...
			dev_debug_printf("m_ncDataCodeWordBit中文 = %d\r\n", m_ncDataCodeWordBit);
		}*/
	}
	
	//dev_debug_printf("m_ncDataCodeWordBit = %d\r\n", m_ncDataCodeWordBit);
	
	return (m_ncDataCodeWordBit != -1);		//(m_ncDataCodeWordBit != -1)
}

/*************************************
 *确定编码的版本号
 *nVersion:确定版本组的开始版本号
 *nVerGroup:版本组，0:1~9 1:10~26 2:27~40
 ************************************/
int GetEncodeVersion(int nVersion, char *lpsSource, int ncLength)
{
	...
	for (i = nVerGroup; i <= QR_VRESION_L; ++i)		//nVerGroup		
	{
		if (EncodeSourceData(lpsSource, ncLength, i))	//在各版本组下计算数据码位数
		{
			/****************************************************
			 *确定编码的版本号
			 *策略是版本号从小到大，比较确定纠错等级下的数据码字大小
			 ****************************************************/
			dev_debug_printf("m_ncDataCodeWordBit11 = %d\r\n", m_ncDataCodeWordBit);
			
			if (i == QR_VRESION_S)		//版本组0，1~9
			{
				for (j = 1; j <= 9; ++j)
				{
					if ((m_ncDataCodeWordBit + 7) / 8 <= QR_VersonInfo[j].ncDataCodeWord[m_nLevel])	//7，剩余位						
						return j;
				}
			}
			else if (i == QR_VRESION_M)		//版本组1，10~26
			{
				for (j = 10; j <= 26; ++j)
				{
			...
			else if (i == QR_VRESION_L)		//版本组2，27~40
			{
				for (j = 27; j <= 40; ++j)
				{
					if ((m_ncDataCodeWordBit + 7) / 8 <= QR_VersonInfo[j].ncDataCodeWord[m_nLevel])						
...
{
	int i, j;
	int nVersion=1;		//开始推算的版本号
	int bAutoExtent=1;
	int ncSource=0;
	int ncLength; 
	int nEncodeVersion;		//确定使用的版本号 
	int ncDataCodeWord;		//数据码个数 
	int ncTerminater;	
	int nDataCwIndex = 0;	//数据码的索引 
	int ncBlock1; 
	int ncBlock2; 
	int ncBlockSum; 
	int nBlockNo = 0;
	...
	unsigned char	byPaddingCode = 0xec;	//补齐符,0xec和0x11
	
	m_nLevel = RSLevel;		//纠错等级，已确定   0
	m_nMaskingNo = 0;		//蒙版参数

    *SymbleSize = 0;
	
	ncLength = ncSource > 0 ? ncSource : strlen(lpsSource);		//计算要编码数据的长度	
	
	dev_debug_printf("ncLength = %d\r\n", ncLength);	
	
	if (ncLength == 0)
	{
		dev_debug_printf("%s(%d):FALSE!\r\n", __FUNCTION__, __LINE__);  
		return FALSE; 
	}

	nEncodeVersion = GetEncodeVersion(nVersion, lpsSource, ncLength);	//确定编码的版本号	
	
	dev_debug_printf("nEncodeVersion = %d\r\n", nEncodeVersion);
	
	if ((nEncodeVersion == 0) || (nEncodeVersion > MAX_VERSION))
	{
		dev_debug_printf("%s(%d):Err(nEncodeVersion=%d)!\r\n", __FUNCTION__, __LINE__, nEncodeVersion);  
		return FALSE; 
	}
	...
	ncDataCodeWord = QR_VersonInfo[m_nVersion].ncDataCodeWord[m_nLevel];	//确定版本和纠错等级下的数据码大小

	ncTerminater = min(4, (ncDataCodeWord * 8) - m_ncDataCodeWordBit);	

	if (ncTerminater > 0)
		m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 0, ncTerminater);	//补0，4个0是结束符	

	/*加上补齐符，不断的重复0xec和0x11*/
	for (i = (m_ncDataCodeWordBit + 7) / 8; i < ncDataCodeWord; ++i)	//7,剩余位
	{
	...
	m_ncAllCodeWord = QR_VersonInfo[m_nVersion].ncAllCodeWord;	//码字总数
	
	memset(m_byAllCodeWord, 0, m_ncAllCodeWord);

	ncBlock1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[m_nLevel].ncRSBlock;	  //纠错的块数
	ncBlock2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[m_nLevel].ncRSBlock;
	ncBlockSum = ncBlock1 + ncBlock2;	//纠错块的总数

	ncDataCw1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[m_nLevel].ncDataCodeWord;	//数据码个数
	ncDataCw2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[m_nLevel].ncDataCodeWord;

	/*复制数据码数据到m_byAllCodeWord*/
	...
	ncRSCw1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[m_nLevel].ncAllCodeWord - ncDataCw1;	//纠错码字个数
	ncRSCw2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[m_nLevel].ncAllCodeWord - ncDataCw2;

	nDataCwIndex = 0;
	nBlockNo = 0;

	/*复制纠错码数据到m_byAllCodeWord*/
	for (i = 0; i < ncBlock1; ++i)
	{
		...
		GetRSCodeWord(m_byRSWork, ncDataCw1, ncRSCw1);	//获取纠错码
	...
	m_nSymbleSize = m_nVersion * 4 + 17;	//正方形的边长

	dev_debug_printf("m_nSymbleSize = %d\r\n", m_nSymbleSize);

	FormatModule();		//设置二维码图案
	
    *SymbleSize = m_nSymbleSize;
...
int dev_Generate_QRencode(char *lpsSource, int RSLevel, u8 x, u8 y)
{
	...
	//m_byModuleData = malloc2D(); 
		
	if( EncodeData(lpsSource, RSLevel, &lSymbleSize) == TRUE )
	{
		/*画二维码的过程*/
		for(i=0; i<lSymbleSize; i++)	//画x方向
        {        				
			if(lSymbleSize<32)	//版本1~3
            {
                for(j=0; j<lSymbleSize; j++)	//画y方向
                {                    
					/*每次画相邻的4个点*/
/*					dev_lcd_set_pixel(m_byModuleData[i][j],(i*2)+x,  (j*2)+y);		//(i*2)+20,(j*2)+2
                    dev_lcd_set_pixel(m_byModuleData[i][j],(i*2)+x,  (j*2)+y+1);
                    dev_lcd_set_pixel(m_byModuleData[i][j],(i*2)+x+1,(j*2)+y);		//(i*2)+21,(j*2)+2	
                    dev_lcd_set_pixel(m_byModuleData[i][j],(i*2)+x+1,(j*2)+y+1);
*/                }
            }
            else	//版本4及以上
            {
                for(j=0; j<lSymbleSize; j++)	//画y方向
                {                     
//                    dev_lcd_set_pixel(m_byModuleData[i][j],i+x, j+y);	//i+32,j+40
                }                    
            }
        }		
...
//		dev_lcd_brush_screen();	
    ...
    {
		dev_debug_printf("EncodeData Error!\r\n");
    }
	
	//free2D(m_byModuleData); 
...
static void dev_prt_line_set_pixel(u8 color, s32 x)
{

    if(color)
    {
        g_line_buf[x>>3] |= (1<<(7-(x&0x07)));
    }
    else
    {
        g_line_buf[x>>3] &= (~(1<<(7-(x&0x07))));
...
/**
 * @brief 二维码打印
 * @param[in] lpsSource 需要生成二维码的内容 
 * @param[in] RSLevel 容错等级 从0 到 3
 * @param[in] msize 二维码放大倍数
 * @param[in] align 二维码对齐格式   0 左对齐， 1 居中， 2 右对齐
 * @retval  TRUE 成功
 * @retval  FALSE 失败
 */
bool print_QrcodeData(char *lpsSource, uint8_t RSLevel, uint8_t msize, uint8_t align)
{
    int i, j, k;
    int lSymbleSize=0;
    uint16_t offset;

    if( EncodeData(lpsSource, RSLevel, &lSymbleSize) == TRUE )	//lSymbleSize:正方形的边长     大趋智能科技有限公司abc123		     
    {   
        dev_debug_printf("00 lSymbleSize = %d\r\n",lSymbleSize);
        if(msize*lSymbleSize > LineDot)   //大于打印机打印点数，退出
        {
            dev_debug_printf("---- 01 , %d \r\n",msize*lSymbleSize);
            return FALSE;
        }
        dev_debug_printf("---- 01 , %d \r\n",msize*lSymbleSize);
        ...
        {
            offset = (LineDot - lSymbleSize*msize)/2;
        }
        else if(align == 2)   //右对齐
        {
            offset = LineDot - lSymbleSize*msize;
        }
		/*画二维码的过程*/
		for(i=0; i<lSymbleSize; i++)	//画x方向
        {
            ...
            {                    
                for(k = 0; k < msize; k++)
                {
                    dev_prt_line_set_pixel(m_byModuleData[i][j], (j*msize) + k + offset );
                }
            }
            dev_debug_printf("\r\n");
            for(k = 0; k < msize; k++)
            ...
            dev_debug_printformat(NULL, g_line_buf, sizeof(g_line_buf));
        }
        return TRUE;
    }
...
void test_qrcode(void)
{
    ...
    if( EncodeData("大趋智能科技有限公司", 0, &lSymbleSize) == TRUE )	//lSymbleSize:正方形的边长     大趋智能科技有限公司abc123		     
    {   
        dev_debug_printf("lSymbleSize = %d\r\n",lSymbleSize);
		/*画二维码的过程*/
		for(i=0; i<lSymbleSize; i++)	//画x方向
        {
            memset(g_line_buf, 0, sizeof(g_line_buf));
			if(lSymbleSize<64)	//版本1~3
            {
                for(j=0; j<lSymbleSize; j++)	//画y方向
                {                    
					/*每次画相邻的4个点*/
/*					dev_lcd_set_pixel(m_byModuleData[i][j],(i*2)+32,(j*2)+40);	//(i*2)+20,(j*2)+2
                    dev_lcd_set_pixel(m_byModuleData[i][j],(i*2)+32,(j*2)+41);
                    dev_lcd_set_pixel(m_byModuleData[i][j],(i*2)+33,(j*2)+40);	//(i*2)+21,(j*2)+2	
                    dev_lcd_set_pixel(m_byModuleData[i][j],(i*2)+33,(j*2)+41);
*/
  /*                  dev_prt_line_set_pixel(m_byModuleData[i][j], (j*4)+40);
                    dev_prt_line_set_pixel(m_byModuleData[i][j], (j*4)+41);
                    dev_prt_line_set_pixel(m_byModuleData[i][j], (j*4)+42);
                    dev_prt_line_set_pixel(m_byModuleData[i][j], (j*4)+43);
                    dev_debug_printf("%02X ",m_byModuleData[i][j]);
  */
                dev_prt_line_set_pixel(m_byModuleData[i][j], (j*4)+40);
                dev_prt_line_set_pixel(m_byModuleData[i][j], (j*4)+41);
                dev_prt_line_set_pixel(m_byModuleData[i][j], (j*4)+42);
            ...
            else	//版本4及以上
            {
                for(j=0; j<lSymbleSize; j++)	//画y方向
                {                     
//                    dev_lcd_set_pixel(m_byModuleData[i][j],i+32, j+40);
                    dev_prt_line_set_pixel(m_byModuleData[i][j], j+40);
                    dev_debug_printf("%02X ",m_byModuleData[i][j]);
