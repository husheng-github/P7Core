...
StackType_t *pxPortInitialiseStack( StackType_t *pxTopOfStack, TaskFunction_t pxCode, void *pvParameters )
{
	...
	*pxTopOfStack = ( StackType_t ) pvParameters;	/* R0 */
	pxTopOfStack -= 9;	/* R11, R10, R9, R8, R7, R6, R5, R4 and R1. */
	*pxTopOfStack = 0;	//
...
void vPortEnterCritical( void )
{
	...
	{
	    if (__get_IPSR()) {
            //printf("Current IPSR is 0x%X, VECTACTIVE is 0x%X\n", __get_IPSR(), portNVIC_INT_CTRL_REG);
	    }
		if ((portNVIC_INT_CTRL_REG & 0xff) > 15)
...
__asm int xPortSetJmp(jmpbuf_t *reg)
{
	PRESERVE8
...
__asm void vPortLongJmp(jmpbuf_t *reg, int val)
{
	PRESERVE8
...
#endif
void privilegedASSERT(void)
{
    if (!__get_MSP()) {
        for(;;); /* we are unprivileged */
    }
}
