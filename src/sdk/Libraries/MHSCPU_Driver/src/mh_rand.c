...
static uint8_t _compute(uint8_t b, uint8_t factor)
{
    if ((factor & b) == factor) {
        return 0x01;
    } else {
...
uint32_t mh_frequency(uint8_t *au8InBuff, uint32_t u32Bytelen)
{
	uint32_t  i, j, u32TmpBitlen;
    int u32DebugNum;
	double	f, s_obs, p_value, sum, sqrt2 = 1.41421356237309504880;
	static uint8_t u8CmpVal[8] = {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01};
    u32TmpBitlen = u32Bytelen << 3;
    sum = 0.0;
    ...
    for(i = 0; i < u32Bytelen; i ++)
    {
        for(j = 0; j < 8; j++)
        {
            u32DebugNum = (int)(_compute(au8InBuff[i], u8CmpVal[j]));
            sum += (2*u32DebugNum - 1);
        }
    } 
    //printf("value of sum is : %lf\n", sum);
    
	s_obs = fabs(sum)/sqrt(u32TmpBitlen);
	f = s_obs/sqrt2;
    //printf("value of f is: %lf\n", f);
    
	p_value = erfc(f);
    //printf("p_value is %lf\n", p_value);
...
uint32_t mh_rand_check(void *rand, uint32_t bytes)
{
    ...
    if (ret == MH_RET_RAND_CHECK_SUCCESS)
        if (mh_frequency((uint8_t*)rand, bytes))
...
#define MH_RAND_USE_MIX_TRNG                            1

    /************ bit definition for TRNG RNG_AMA REGISTER ************/
#define MH_TRNG_RNG_AMA_PD_TRNG0_Mask                   (BIT(12)|BIT(13)|BIT(14)|BIT(15))               
...
//  int n = 0;
    ...
    dword *p = (dword *)rand;       //This may cause fault when rand is not aligned.
