...
/* Sets up Diffie-Hellman key agreement. Public value has same length
   as prime.
 */
int R_SetupDHAgreement(publicValue, privateValue, privateValueLen, params, randomStruct)
...
R_RANDOM_STRUCT *randomStruct;                          /* random structure */
{
     ...
     */
    status = R_GenerateBytes (privateValue, privateValueLen, randomStruct);
    if (0 != status)
        return (status);
 ...
 */
int R_ComputeDHAgreedKey(agreedKey, otherPublicValue, privateValue, privateValueLen, params)
