...
static NN_DIGIT NN_AddDigitMult PROTO_LIST((NN_DIGIT *, NN_DIGIT *, NN_DIGIT, NN_DIGIT *, unsigned int));
static NN_DIGIT NN_SubDigitMult PROTO_LIST((NN_DIGIT *, NN_DIGIT *, NN_DIGIT, NN_DIGIT *, unsigned int));
...
unsigned int digits;
{
    ...
    t = (NN_DIGIT *)pcicorecallfunc.mem_malloc(2*MAX_NN_DIGITS*sizeof(NN_DIGIT));
    ...
    R_memset ((POINTER)t, 0, sizeof (NN_DIGIT)*2*MAX_NN_DIGITS);
...
unsigned int cDigits, dDigits;
{
    ...
    /* Normalize operands.
     */
    cc = (NN_DIGIT *)pcicorecallfunc.mem_malloc(sizeof(NN_DIGIT)*(2*MAX_NN_DIGITS+1));
	if(cc == NULL)
	{
		return;
	}
	
	dd = (NN_DIGIT *)pcicorecallfunc.mem_malloc(sizeof(NN_DIGIT)*MAX_NN_DIGITS);
	if(dd == NULL)
	{
	    pcicorecallfunc.mem_free(cc);
		return;
	}
    ...
    /* Zeroize potentially sensitive information.
     */
    R_memset ((POINTER)cc, 0, sizeof(NN_DIGIT)*(2*MAX_NN_DIGITS+1));
    R_memset ((POINTER)dd, 0, sizeof(NN_DIGIT)*MAX_NN_DIGITS);

	pcicorecallfunc.mem_free(cc);
	pcicorecallfunc.mem_free(dd);
}
...
unsigned int cDigits, dDigits;
{
	...
	bPower[0] = (NN_DIGIT *)pcicorecallfunc.mem_malloc(MAX_NN_DIGITS*sizeof(NN_DIGIT));
	bPower[1] = (NN_DIGIT *)pcicorecallfunc.mem_malloc(MAX_NN_DIGITS*sizeof(NN_DIGIT));
	bPower[2] = (NN_DIGIT *)pcicorecallfunc.mem_malloc(MAX_NN_DIGITS*sizeof(NN_DIGIT));
	t = (NN_DIGIT *)pcicorecallfunc.mem_malloc(MAX_NN_DIGITS*sizeof(NN_DIGIT));
    ...
    /* Zeroize potentially sensitive information.
     */
    R_memset ((POINTER)bPower[0], 0, MAX_NN_DIGITS*sizeof(NN_DIGIT));
	R_memset ((POINTER)bPower[1], 0, MAX_NN_DIGITS*sizeof(NN_DIGIT));
	R_memset ((POINTER)bPower[2], 0, MAX_NN_DIGITS*sizeof(NN_DIGIT));
    R_memset ((POINTER)t, 0, MAX_NN_DIGITS*sizeof(NN_DIGIT));

	pcicorecallfunc.mem_free(bPower[0]);
	pcicorecallfunc.mem_free(bPower[1]);
	pcicorecallfunc.mem_free(bPower[2]);
	pcicorecallfunc.mem_free(t);
}

#else  //sxl use hardware arith
extern unsigned char RSA_ModExp(unsigned char* in, unsigned char* e, unsigned char* n, unsigned char* out, unsigned int ebitlen, unsigned int nbitlen);
void NN_ModExp (a, b, c, cDigits, d, dDigits)
NN_DIGIT *a, *b, *c, *d;
unsigned int cDigits, dDigits;
{
...
unsigned int digits;
{
	...
	*/
	...
	t1 = (NN_DIGIT *)pcicorecallfunc.mem_malloc(sizeof(NN_DIGIT)*MAX_NN_DIGITS);
	if(t1 == NULL)
	{
	    pcicorecallfunc.mem_free(q);
		return;
	}
	t3 = (NN_DIGIT *)pcicorecallfunc.mem_malloc(sizeof(NN_DIGIT)*MAX_NN_DIGITS);
	if(t3 == NULL)
	{
	    pcicorecallfunc.mem_free(q);
		pcicorecallfunc.mem_free(t1);
		return;
	}
	u1 = (NN_DIGIT *)pcicorecallfunc.mem_malloc(sizeof(NN_DIGIT)*MAX_NN_DIGITS);
	if(u1 == NULL)
	{
	    pcicorecallfunc.mem_free(q);
		pcicorecallfunc.mem_free(t1);
		pcicorecallfunc.mem_free(t3);
		return;
	}
	u3 = (NN_DIGIT *)pcicorecallfunc.mem_malloc(sizeof(NN_DIGIT)*MAX_NN_DIGITS);
	if(u3 == NULL)
	{
	    pcicorecallfunc.mem_free(q);
		pcicorecallfunc.mem_free(t1);
		pcicorecallfunc.mem_free(t3);
		pcicorecallfunc.mem_free(u1);
		return;
	}
	v1 = (NN_DIGIT *)pcicorecallfunc.mem_malloc(sizeof(NN_DIGIT)*MAX_NN_DIGITS);
	if(v1 == NULL)
	{
	    pcicorecallfunc.mem_free(q);
		pcicorecallfunc.mem_free(t1);
		pcicorecallfunc.mem_free(t3);
		pcicorecallfunc.mem_free(u1);
		pcicorecallfunc.mem_free(u3);
		return;
	}
	v3 = (NN_DIGIT *)pcicorecallfunc.mem_malloc(sizeof(NN_DIGIT)*MAX_NN_DIGITS);
	if(v3 == NULL)
	{
	    pcicorecallfunc.mem_free(q);
		pcicorecallfunc.mem_free(t1);
		...
		pcicorecallfunc.mem_free(u1);
		pcicorecallfunc.mem_free(u3);
		pcicorecallfunc.mem_free(v1);
		return;
	}
	w = (NN_DIGIT *)pcicorecallfunc.mem_malloc(sizeof(NN_DIGIT)*2*MAX_NN_DIGITS);
	if(w == NULL)
	{
	    pcicorecallfunc.mem_free(q);
		pcicorecallfunc.mem_free(t1);
    ...
    */
    ...
    R_memset ((POINTER)t1, 0, sizeof(NN_DIGIT)*MAX_NN_DIGITS);
    R_memset ((POINTER)t3, 0, sizeof(NN_DIGIT)*MAX_NN_DIGITS);
    R_memset ((POINTER)u1, 0, sizeof(NN_DIGIT)*MAX_NN_DIGITS);
    R_memset ((POINTER)u3, 0, sizeof(NN_DIGIT)*MAX_NN_DIGITS);
    R_memset ((POINTER)v1, 0, sizeof(NN_DIGIT)*MAX_NN_DIGITS);
    R_memset ((POINTER)v3, 0, sizeof(NN_DIGIT)*MAX_NN_DIGITS);
    R_memset ((POINTER)w, 0, sizeof(NN_DIGIT)*2*MAX_NN_DIGITS);
