...
static void EncryptPEMUpdateFinal PROTO_LIST((R_ENVELOPE_CTX *, unsigned char *, unsigned int *, unsigned char *, unsigned int));
static int DecryptPEMUpdateFinal PROTO_LIST((R_ENVELOPE_CTX *, unsigned char *, unsigned int *, unsigned char *, unsigned int));
static int CipherInit PROTO_LIST((R_ENVELOPE_CTX *, int, unsigned char *, unsigned char *, int));
static void CipherUpdate PROTO_LIST((R_ENVELOPE_CTX *, unsigned char *, unsigned char *, unsigned int));
...
R_RSA_PUBLIC_KEY *publicKey;                     /* signer's RSA public key */
{
    ...
    do {
        ...
        if ((originalDigestInfoLen != DIGEST_INFO_LEN) ||
                (R_memcmp((POINTER)originalDigestInfo, (POINTER)digestInfo, DIGEST_INFO_LEN)))
        {
            status = RE_SIGNATURE;
            break;
        }
 ...
 */
int R_SealInit(context, encryptedKeys, encryptedKeyLens, iv, publicKeyCount, publicKeys, encryptionAlgorithm, randomStruct)
...
R_RANDOM_STRUCT *randomStruct;                          /* random structure */
{
    ...
    do {
        ...
        for (i = 0; i < publicKeyCount; ++i) {
            if (RSAPublicEncrypt(encryptedKeys[i], &encryptedKeyLens[i], key, keyLen, publicKeys[i], randomStruct))
            {
                status = RE_PUBLIC_KEY;
                break;
            }
...
unsigned int partInLen;                         /* length of next data part */
{
     ...
     */
    R_memcpy((POINTER)(context->buffer + context->bufferLen), (POINTER)partIn, tempLen);
    ...
    *partOutLen += tempLen;
     ...
     */
    R_memcpy((POINTER)context->buffer, (POINTER)partIn, context->bufferLen = partInLen);
...
int R_SignPEMBlock(encodedContent, encodedContentLen, encodedSignature, encodedSignatureLen, content, contentLen, recode, digestAlgorithm, privateKey)
...
int R_VerifyPEMSignature(content, contentLen, encodedContent, encodedContentLen, encodedSignature, encodedSignatureLen, recode, digestAlgorithm, publicKey)
unsigned char *content;                                          /* content */
unsigned int *contentLen;                              /* length of content */
...
int R_SealPEMBlock(encryptedContent, encryptedContentLen, encryptedKey, encryptedKeyLen, encryptedSignature, encryptedSignatureLen, iv, content, contentLen, digestAlgorithm, publicKey, privateKey, randomStruct)
...
R_RANDOM_STRUCT *randomStruct;                          /* random structure */
{
    ...
    do {
        ...
        R_EncodePEMBlock(encryptedKey, encryptedKeyLen, encryptedKeyBlock, encryptedKeyBlockLen);

        EncryptPEMUpdateFinal(&context, encryptedContent, encryptedContentLen, content, contentLen);

        EncryptPEMUpdateFinal (&context, encryptedSignature, encryptedSignatureLen, signature, signatureLen);
    }
    while (0);
...
int R_OpenPEMBlock(content, contentLen, encryptedContent, encryptedContentLen, encryptedKey, encryptedKeyLen, encryptedSignature, encryptedSignatureLen, iv, digestAlgorithm, privateKey, publicKey)
unsigned char *content;                                          /* content */
unsigned int *contentLen;                              /* length of content */
unsigned char *encryptedContent;              /* encoded, encrypted content */
unsigned int encryptedContentLen;                                 /* length */
...
R_RSA_PUBLIC_KEY *publicKey;                     /* signer's RSA public key */
{
    ...
    do {
        ...
        if ((status = DecryptPEMUpdateFinal (&context, content, contentLen, encryptedContent, encryptedContentLen)) != 0) {
        ...
        status = DecryptPEMUpdateFinal(&context, signature, &signatureLen, encryptedSignature, encryptedSignatureLen);
        if (0 != status)
        {
            if ((status == RE_LEN || status == RE_ENCODING))
        ...
        if ((status = R_VerifyBlockSignature (content, *contentLen, signature, signatureLen, digestAlgorithm, publicKey)) != 0)
            break;
    }
    while (0);
 ...
 */
static void EncryptPEMUpdateFinal(context, output, outputLen, input, inputLen)
R_ENVELOPE_CTX *context;
...
unsigned int len;                      /* length of input and output blocks */
{
    ...
    {
        if (context->encryptionAlgorithm == EA_DESX_CBC)
